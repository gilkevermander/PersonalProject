...
//	public bool NearMode = false;
	...
	public List<AvatarController> Player2Controllers;
...
//				break;
	...
	void Awake()
	{
		...
		if(Player1Avatars.Count == 0 && Player2Avatars.Count == 0 && SceneManager.GetActiveScene().name == "Uitleg")
		{
			AvatarController[] avatars = FindObjectsOfType(typeof(AvatarController)) as AvatarController[];
	...
	void Update()
	{
		if(KinectInitialized)
		{
			...
			{
                Debug.Log("er is een tweede speler gedetecteerd");
...
//					}
	...
	void RemoveUser(uint UserId)
	{
		...
		if(UserId == Player1ID)
		{
            if (CalibrationText != null)
            {
                CalibrationText.text = "hé ben je daar nog?";
            }

            // na 15 seconden terug naar het startscherm
            countdown = true;
